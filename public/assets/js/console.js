...
(() => {
  ...
  const { esc, safeUrl, md, sessionId, verifyActions } = core;

  const ENDPOINT = core.apiBase() + '/v1/chat';
  const history = [];
  const isAr = () => document.documentElement.lang === 'ar';
  const t = (en, ar) => (isAr() ? ar : en);

  /* ---- contract rendering (markdown + badge live in chat-core) ---- */
  // The card top always carries two grid slots — pad the empty badge state.
  const badge = (data) => core.badgeHtml(data) || '<span></span>';
  ...
  /* ---- backend (transport lives in chat-core) ---- */
  ...
  function send(text) {
    ...
    (async () => {
      ...
      history.push({ role: 'model', text: answer || core.errorText() });
      if (sendBtn) sendBtn.disabled = false;
    })().catch((err) => {
      const reply = (err && err.message) === 'rate_limited' ? core.rateLimitedText() : core.errorText();
